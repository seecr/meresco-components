...
# Copyright (C) 2015-2018, 2020-2022, 2024 Seecr (Seek You Too B.V.) https://seecr.nl
...
class JsonSearch(Observable):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._getItems = (
            self._observerGetItems if getItemsFromObserver else self._defaultGetItems
        )
        self._collectLogForScope = (
            collectLogForScope if enableCollectLog else lambda **kwargs: None
        )
                ...
                for fieldname, terms in jsonResponse.get("facets", {}).items():
                    if fieldname.endswith(".uri"):
                        ...
                        def displayValue(term):
                            return (
                                term["displayTerm"]
                                if "displayTerm" in term
                                else self.call.labelForUri(uri=term["term"])
                            )
        ...
        if hits:
            if hasattr(result, "items"):
                ...
                jsonResponse["items"] = yield self._getItems(
                    identifiers=[hit.id for hit in hits], recordSchema=args.recordSchema
                )
    ...
    def _defaultGetItems(self, identifiers, recordSchema):
        ...
        for identifier in identifiers:
            result.append((yield self._retrieveData(identifier, recordSchema)))
        return result

    def _retrieveData(self, identifier, recordSchema):
        try:
            data = yield self.any.retrieveData(identifier=identifier, name=recordSchema)
        except Exception:
            return {
                "identifier": identifier,
                "error": f"The recordSchema '{recordSchema}' does not exist.",
            }
        return data

    def _observerGetItems(self, identifiers, recordSchema):
        return [
            self._observerGetItem(identifier, recordSchema)
            for identifier in identifiers
        ]
        yield

    def _observerGetItem(self, identifier, recordSchema):
        try:
            return self.call.getItem(identifier=identifier, recordSchema=recordSchema)
        except Exception:
            return {
                "identifier": identifier,
                "error": f"The recordSchema '{recordSchema}' does not exist.",
...
class _Arguments(object):
    def __init__(
        ...
        if "sort" in arguments:
            sort = [
                s.strip()
                for sortOption in arguments.pop("sort")
                for s in sortOption.split(",")
                if s.strip()
            ]
            sortKeys = []
            for key in sort:
                ...
                if key:
                    ...
                    self._request.setdefault("sort", []).append(
                        ("" if ascending else "-") + key
                    )
                    self._next_request.setdefault("sort", []).append(
                        ("" if ascending else "-") + key
                    )
        ...
        for facet in facets:
            ...
            try:
                maxTerms = (
                    int(splitted[1]) if len(splitted) > 1 else default_facet_terms_count
                )
            ...
            facet_term_count = (
                ":{0}".format(maxTerms) if maxTerms != default_facet_terms_count else ""
            )
            self._next_request.setdefault("facet", []).append(
                fieldname + facet_term_count
            )
        ...
        if facetFilters:
            ...
            for facetFilter in facetFilters:
                ...
                q.operands.append(
                    QueryExpression.searchterm(index=index, relation="exact", term=term)
                )
    ...
    def pageDict(self, pageAddition, path):
        ...
        return dict(
            page=page,
            link="{0}?{1}".format(
                path, urlencode(sorted(dict(self._next_request, page=page).items()))
            ),
        )
...
class BadArgument(ValueError):
    def __init__(self, argument):
        ValueError.__init__(
            self, "Bad argument: {} is an illegal parameter".format(repr(argument))
        )
...
_first = {
    key: nr
    for nr, key in enumerate(
        reversed(["total", "items", "response", "request"]), start=1
    )
}
