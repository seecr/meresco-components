...
from lxml.etree import parse, _ElementUnicodeResult, XML
...
class XmlPumpTest(SeecrTestCase):
    def setUp(self):
        ...
        self.observable = be(
            (
                Observable(),
                (XmlParseLxml(fromKwarg="data", toKwarg="lxmlNode"), (self.observer,)),
            )
        )
    ...
    def testParse2(self):
        ...
        list(
            compose(
                self.observable.all.add(
                    identifier="id", partname="partName", data=xmlString
                )
            )
        )
        list(
            compose(
                self.observable.any.add(
                    identifier="id", partname="partName", data=xmlString
                )
            )
        )
    ...
    def testParseWithElementStringResult(self):
        xmlString = """<tag><content>contents</content></tag>""".encode(
            encoding="utf-8"
        )
        self.observable.do.add(identifier="id", partname="partName", data=xmlString)
    ...
    def testParseWithParseOptions(self):
        ...
        self.observable.do.add(identifier="id", partname="partName", data=xmlString)
        self.assertEqual(
            xmlString, lxmltostring(self.observer.calledMethods[0].kwargs["lxmlNode"])
        )
            ...
            (
                ...
                (
                    XmlParseLxml(
                        fromKwarg="data",
                        toKwarg="lxmlNode",
                        parseOptions=dict(remove_blank_text=True, ns_clean=True),
                    ),
                    (self.observer,),
                ),
            )
        ...
        self.observable.do.add(identifier="id", partname="partName", data=xmlString)
        self.assertEqual(
            """<tag xmlns:xyz="uri:xyz"><content>contents</content></tag>""",
            lxmltostring(self.observer.calledMethods[1].kwargs["lxmlNode"]),
        )
    ...
    def testXmlPrintLxml(self):
        ...
        list(
            compose(
                observable.all.someMessage(lxmlNode=parse(StringIO("<a><b>“c</b></a>")))
            )
        )
...
""",
    ...
    def testXmlPrintLxmlPrettyPrintFalse(self):
        observable = Observable()
        xmlprintlxml = XmlPrintLxml(
            fromKwarg="lxmlNode", toKwarg="data", pretty_print=False
        )
        observer = CallTrace("observer", emptyGeneratorMethods=["someMessage"])
        xmlprintlxml.addObserver(observer)
        observable.addObserver(xmlprintlxml)
        ...
        self.assertEqual(["data"], list(observer.calledMethods[0].kwargs.keys()))
        self.assertEqual(
            b"""<a><b>\xe2\x80\x9cc</b></a>""", observer.calledMethods[0].kwargs["data"]
        )
    ...
    def testTransparency(self):
        ...
        observable.do.something(
            identifier="identifier",
            partname="partName",
            data='<?xml version="1.0"?><a><b>c</b></a>',
        )
        self.assertEqualsWS(
            "<a><b>c</b></a>", lxml.calledMethods[0].kwargs["data"].decode()
        )
        self.assertEqualsWS(
            "<a><b>c</b></a>", lxml2.calledMethods[0].kwargs["data"].decode()
        )
    ...
    def testMissingFromKwargDoesNothing(self):
        ...
        observable = be(
            (
                Observable(),
                (XmlPrintLxml(fromKwarg="lxmlNode", toKwarg="data"), (observer,)),
            )
        )
    ...
    def testRenameKwargOnConvert(self):
        ...
        observable.do.something(
            "identifier", "partname", lxmlNode=parse(StringIO("<someXml/>"))
        )
        self.assertEqual(
            r"something('identifier', 'partname', dataString=b'<someXml/>\n')",
            str(observer.calledMethods[0]),
        )

        observable.do.something("identifier", "partname", someKwarg=1)
        self.assertEqual(
            "something('identifier', 'partname', someKwarg=1)",
            str(observer.calledMethods[1]),
        )
    ...
    def testToKwargDefaultsToFromKwarg(self):
        ...
        observable.do.something(
            "identifier", "partname", data=parse(StringIO("<someXml/>"))
        )
        self.assertEqual(
            r"something('identifier', 'partname', data=b'<someXml/>\n')",
            str(observer.calledMethods[0]),
        )
    ...
    def testLxmltostringFixes(self):
        ...
        self.assertEqual(
            b"<root><sub ...", _fixLxmltostringRootElement(b"<root><sub ...")
        )
        self.assertEqual(
            b'<root attrib="aap&amp;noot"><sub ...',
            _fixLxmltostringRootElement(b'<root attrib="aap&amp;noot"><sub ...'),
        )
        self.assertEqual(
            b'<root attrib="aap&euro;noot"><sub ...',
            _fixLxmltostringRootElement(b'<root attrib="aap&euro;noot"><sub ...'),
        )
        self.assertEqual(
            '<root attrib="ĳs"><sub ...'.encode(),
        ...
        self.assertEqual(
            '<root attrib="ĳs"><sub attrib="&#x133;s">...'.encode(),
            _fixLxmltostringRootElement(
                b'<root attrib="&#x133;s"><sub attrib="&#x133;s">...'
            ),
        )
