...
#
# Copyright (C) 2014-2015, 2018, 2020, 2026 Seecr (Seek You Too B.V.) https://seecr.nl
# Copyright (C) 2014 Stichting Bibliotheek.nl (BNL) http://www.bibliotheek.nl
# Copyright (C) 2014, 2020, 2026 Stichting Kennisnet https://www.kennisnet.nl
...
class DirectoryLogTest(SeecrTestCase):
    ...
    def testNewDayNewLogFile(self):
        ...
        log.log(**kwargs)
        self.assertEqual(
            ["2009-11-02-query.log", "2009-11-03-query.log"],
    ...
    def testRemoveOldLogs(self):
        ...
        log = DirectoryLog(
            self.tempdir, extension="-the-end.log", nrOfFilesKept=nrOfFilesKept
        )
        log.log(**kwargs)
    ...
    def testListLogs(self):
        ...
        log = DirectoryLog(self.tmp_path)
        log.log(timestamp=timestamp)
        log.log(timestamp=timestamp + 1)
        log.log(timestamp=timestamp + oneday)
        log.log(timestamp=timestamp + 2 * oneday)
        self.assertEqual(
            ["2009-11-02-query.log", "2009-11-03-query.log", "2009-11-04-query.log"],
            log.listlogs(),
        )
        self.assertEqual(
            ["2009-11-02-query.log.gz", "2009-11-03-query.log", "2009-11-04-query.log"],
            sorted(f.name for f in self.tmp_path.iterdir()),
        )

    def testGzipOnInit(self):
        for name in [
            "2009-11-02-query.log",
            "2009-11-03-query.log",
            "2009-11-04-query.log",
        ]:
            self.tmp_path.joinpath(name).write_text(f"{name} line\n")
        log = DirectoryLog(self.tmp_path)
        self.assertEqual(
            ["2009-11-02-query.log", "2009-11-03-query.log", "2009-11-04-query.log"],
            log.listlogs(),
        ...
        self.assertEqual(
            [
                "2009-11-02-query.log.gz",
                "2009-11-03-query.log.gz",
                "2009-11-04-query.log",
            ],
            sorted(f.name for f in self.tmp_path.iterdir()),
        )
        ...
        self.assertEqual(
            "2009-11-02-query.log line\n", next(log.getlog("2009-11-02-query.log"))
        )
        self.assertEqual(
            "2009-11-03-query.log line\n", next(log.getlog("2009-11-03-query.log"))
        )

    def testWriteOldLog(self):
        oneday = 24 * 60 * 60
        timestamp = 1257161136.0
        log = DirectoryLog(self.tempdir)
        log.log(timestamp=timestamp)
        ...
        self.assertRaises(ValueError, lambda: log.log(timestamp=timestamp + 1))
        log.log(timestamp=timestamp + 2 * oneday + 1)
