...
# Copyright (C) 2015-2017, 2020-2022, 2024 Seecr (Seek You Too B.V.) https://seecr.nl
# Copyright (C) 2020 Data Archiving and Network Services https://dans.knaw.nl
# Copyright (C) 2020 SURF https://www.surf.nl
# Copyright (C) 2020-2022, 2024 Stichting Kennisnet https://www.kennisnet.nl
...
class JsonSearchTest(SeecrTestCase):
    def setUp(self):
        ...
        self.observer = CallTrace(
            methods=dict(executeQuery=executeQuery, retrieveData=retrieveData)
        )
        self.logs = []
        self._buildDna()
    ...
    def _buildDna(self, defaultRecordSchema="rdf", **kwargs):
        jsonSearch = JsonSearch(
            defaultRecordSchema=defaultRecordSchema, enableCollectLog=True, **kwargs
        )
    ...
    def testRecords(self):
        json = self.request()
        self.assertEqual(
            ["executeQuery", "retrieveData", "retrieveData"],
    ...
    def testTimes(self):
        json = self.request()
        self.assertEqual(
            {"handlingTime": 0.2, "indexTime": 0.03, "queryTime": 0.1},
            json["response"]["querytimes"],
        )
        self.indexQueryTime = 0
        json = self.request()
        self.assertEqual(
            {"handlingTime": 0.2, "indexTime": 0, "queryTime": 0.1},
            json["response"]["querytimes"],
        )
    ...
    def testQuery(self):
        ...
        self.assertEqual(
            QueryExpression.searchterm(term="*"), executeQueryMethod.kwargs["query"]
        )
    ...
    def testQueryNotCQLButWebQueryStyle(self):
        ...
        executeQueryMethod = self.observer.calledMethods[0]
        self.assertEqual(
            cqlToExpression("fiets AND water"), executeQueryMethod.kwargs["query"]
        )
    ...
    def testFacet(self):
        ...
        self.assertEqual(
            [{"index": "field", "max-terms": 100}], json["request"]["facet"]
        )
    ...
    def testFacetDisplayValue(self):
        ...
        executeQueryMethod = self.observer.calledMethods[0]
        self.assertEqual(
            [{"fieldname": "field:test.uri", "maxTerms": 10, "sortBy": "count"}],
            executeQueryMethod.kwargs["facets"],
        )
    ...
    def testMultipleFacets(self):
        ...
        self.assertEqual(
            [{"index": "field0", "max-terms": 10}, {"index": "field1", "max-terms": 3}],
            json["request"]["facet"],
        )
        self.assertEqual(
            set(["field0", "field1"]), set(json["response"]["facets"].keys())
        )

    def testDataNotAvailable(self):
        ...
        self.assertEqual(
            ...
            [
                {
                    "identifier": "id:1",
                    "error": f"The recordSchema 'rdf' does not exist.",
                },
                {"identifier": "id:2", "name": "rdf"},
            ],
        )
        self.assertEqual(
            ["executeQuery", "retrieveData", "retrieveData"],
    ...
    def testDataNotAvailable_getItem(self):
        class MyException(Exception):
            pass

        def getItem(identifier, recordSchema):
            if identifier == "id:1":
                raise MyException()
            return {"identifier": identifier, "recordSchema": recordSchema}

        self.observer.methods["getItem"] = getItem
        ...
        self.assertEqual(
            ...
            [
                ...
                {"identifier": "id:2", "recordSchema": "rdf"},
            ],
        )
        self.assertEqual(
            ["executeQuery", "getItem", "getItem"], self.observer.calledMethodNames()
        )
    ...
    def testFacetFilter(self):
        ...
        self.assertEqual(
            sorted(["field=somevalue", "field=value0"]),
            sorted(link.query["facet-filter"]),
        )
    ...
    def testPassXArguments(self):
        ...
        response = self.request(
            **{"x-disable-filter": "encyclopedie", "x-something-else": "important"}
        )
        executeQueryMethod = self.observer.calledMethods[0]
        self.assertEqual(
            {
    ...
    def testFacetFilters(self):
        self.request(
            facet="field", **{"facet-filter": ["field0=value0", "field1=value1"]}
        )
        executeQueryMethod = self.observer.calledMethods[0]
        self.assertEqual(
            cqlToExpression("* AND field0 exact value0 AND field1 exact value1"),
            executeQueryMethod.kwargs["query"],
        )
    ...
    def testLinksWithOtherPath(self):
        ...
        self.assertEqual(
            "/path/to/search",
            self.parseLink(response["response"]["previous"]["link"]).path,
        )
        self.assertEqual(
            "/path/to/search", self.parseLink(response["response"]["next"]["link"]).path
        )
        self.assertEqual(
            "/path/to/search",
            self.parseLink(response["response"]["facets"]["field"][0]["link"]).path,
        )
    ...
    def testGetItemsFromObserver(self):
        ...
        getItem = self.observer.calledMethods[-1]
        self.assertEqual(
            dict(identifier="id:2", recordSchema="another"), getItem.kwargs
        )

    def testParameterQueryOmitted(self):
        response = self.request(facet="field", query=None)
        self.assertEqual(
            dict(type="MissingArgument", message="Missing required argument: 'query'"),
            response["error"],
        )
    ...
    def testParameterQueryWebQuery(self):
        ...
        executeQueryMethod = self.observer.calledMethods[0]
        self.assertEqual(
            cqlToExpression("value1 AND value2"), executeQueryMethod.kwargs["query"]
        )
        ...
        executeQueryMethod = self.observer.calledMethods[0]
        self.assertEqual(
            cqlToExpression("* NOT value"), executeQueryMethod.kwargs["query"]
        )
    ...
    def testBadArguments(self):
        ...
        self.assertEqual(
            {
                "type": "BadArgument",
                "message": "Bad argument: 'unkown_parameter' is an illegal parameter",
            },
            response["error"],
        )
    ...
    def testSequenceOfKeys(self):
        ...
        self.assertEqual(["response", "request", "version"], list(json.keys()))
        self.assertEqual(
            ["total", "items", "facets", "querytimes", "next", "previous"],
    ...
    def request(self, **kwargs):
        ...
        arguments = {"query": kwargs.pop("query", "*")}
        arguments.update(kwargs)
        arguments = parse_qs(
            urlencode(
                {k: v for k, v in list(arguments.items()) if v is not None}, doseq=True
            )
        )
        requestDict["arguments"] = arguments
        header, body = asString(self.dna.all.handleRequest(**requestDict)).split(
            CRLF * 2, 1
        )
